...
import useFechSearch from '@/hooks/useFechSearch'
...
function ProgramSearch({formValue}: Props) {

    const { data, isLoading ,error }  = useFechSearch(formValue?.name);    
  ...
  return (
    <div className='text-white mr-4 w-full col-span-3  md:col-span-2'>
        {formValue.name === "" 
            ? (
                <div className='flex justify-center items-center w-full h-full'>
                    <p className='text-lg font-bold text-black dark:text-white'>No hay resultado</p>
                </div>
            )
            : (
            <ul className='text-black dark:text-white flex gap-3 flex-wrap justify-center'>
                {
                    (
                    data?.map((item,index) => (
                        <li key={index + 555} className=' flex flex-col w-max px-10 shadow-lg rounded-md bg-white/20 backdrop-blur-md py-2 border-2 border-white/20'>
                            <div className='flex flex-col justify-center items-center gap-2'>
                                ...
                                <p className='text-center px-4 py-1 text-sm'>{convertFecha(item.horaFin).toLocaleString()}</p>
                            </div>
                        </li> 
                    ))
                )
